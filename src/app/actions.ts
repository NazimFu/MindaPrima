...
import {
  getOrCreateSnapshot,
  updateSnapshotStudent,
  setSnapshotPaymentStatus,
  monthKeyFromString,
...
/**
 * Convert a snapshot student back to the Student shape the UI expects.
 * Active = false students are excluded so they never appear on invoices.
 */
function snapToStudent(s: SnapshotStudent): Student {
  return {
    id:              s.studentId,
    name:            s.name,
    level:           s.level,
...
/**
 * Returns students for a given month. If no snapshot exists it is created
 * automatically by carrying forward from the previous month.
 *
 * @param selectedMonth "current" | "March 2025"
 */
export async function getStudentsForMonth(selectedMonth: string): Promise<Student[]> {
  try {
    const key       = monthKeyFromString(selectedMonth);
    const snapshots = await getOrCreateSnapshot(key);
    return snapshots
      .filter(s => s.active)
      .map(snapToStudent);
  } catch (error) {
    console.error('Error fetching monthly students:', error);
    // Fallback to master sheet so the UI never breaks
    return getStudents();
  }
}

/**
 * Update the subjects for one student in a specific month snapshot.
 * Called from the invoice page when the teacher edits a student's subjects.
 */
export async function updateStudentSubjectsForMonth(
  selectedMonth: string,
  studentId:     string,
  subjects:      string,
) {
  const key = monthKeyFromString(selectedMonth);
  await updateSnapshotStudent(key, studentId, { subjects });
  revalidate();
}

/**
 * Mark a student as inactive (not continuing) for a given month.
 */
export async function deactivateStudentForMonth(
  selectedMonth: string,
  studentId:     string,
) {
  const key = monthKeyFromString(selectedMonth);
  await updateSnapshotStudent(key, studentId, { active: false });
  revalidate();
}

/**
 * Mark a student as active again for a given month (undo deactivation).
 */
export async function reactivateStudentForMonth(
  selectedMonth: string,
  studentId:     string,
) {
  const key = monthKeyFromString(selectedMonth);
  await updateSnapshotStudent(key, studentId, { active: true });
  revalidate();
}

/**
 * Update payment status for a student in the month snapshot AND in the
 * master Students sheet (so the dashboard stat card stays accurate).
 */
export async function updateStudentStatusForMonth(
  selectedMonth: string,
  studentId:     string,
  status:        PaymentStatus,
) {
  const key = monthKeyFromString(selectedMonth);
  await setSnapshotPaymentStatus(key, studentId, status);

  // Also update master sheet
  const sheet = await getSheet(STUDENT_SHEET_NAME);
  if (sheet) {
    const rows = await getSheetData(sheet);
    const rowIndex = rows.findIndex(r => r[0] === studentId);
    if (rowIndex > -1) {
      const rowToUpdate = rows[rowIndex];
      rowToUpdate[9] = status;
...
export async function getPrices(): Promise<Prices> {
  ...
  try {
    ...
    if (!pricesSheet) return defaultPrices;
    const data = await getSheetData(pricesSheet, true);
    if (data.length < 2) return defaultPrices;
    ...
    if (tiIndex > -1 && pricesData[0]?.[tiIndex])
      prices.transportInbound  = parseFloat(pricesData[0][tiIndex]) || 0;
    if (toIndex > -1 && pricesData[0]?.[toIndex])
    ...
    header.forEach((colName: string, index: number) => {
      if (levelShortMap[colName]) levelColMap[levelShortMap[colName]] = index;
    });
    ...
    pricesData.forEach(row => {
      ...
      for (const levelName in levelColMap) {
        ...
        if (!isNaN(price)) {
          if (!prices[levelName as StudentLevel]) prices[levelName as StudentLevel] = {};
...
export async function updatePrices(prices: Prices) {
  ...
  await batchUpdateSheet(PRICES_SHEET_NAME, (sheetData) => {
    ...
    if (qtySubjectsIndex === -1) header[0] = 'qtySubjects';
    ...
    for (const level of Object.keys(prices)) {
      ...
      if (colIndex === undefined) { colIndex = header.length; header.push(shortLevel); levelColMap[level] = colIndex; }
...
// ── Master student CRUD (Students sheet) ─────────────────────────────────────
// These still write to the master sheet. They are used for registering NEW
// students and editing permanent details (name, guardian, address etc.).
...
export async function addTeacher(data: Omit<Teacher, 'id'>) {
  ...
  const newRow = [newId, validatedData.name, validatedData.subject, validatedData.contact];
...
export async function updateTeacher(teacher: Teacher) {
  ...
  const updatedRow    = [teacher.id, validatedData.name, validatedData.subject, validatedData.contact];
